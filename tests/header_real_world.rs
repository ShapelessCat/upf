...
fn header_accepts_true_false_aliases() {
    ...
    <UPF version="2.0.1">
      ...
      <PP_FULL_WFC>
        <PP_AEWFC.1 index="1" label="5F" l="3">0.0</PP_AEWFC.1>
        <PP_PSWFC.1 index="1" label="5F" l="3">0.0</PP_PSWFC.1>
      </PP_FULL_WFC>
    ...
    assert_eq!(doc.header.total_psenergy, -1.0);
    assert_eq!(doc.paw.as_ref().unwrap().core_energy, 0.0);
}
...
fn header_accepts_uspp_alias_when_present() {
    ...
    assert_eq!(doc.header.total_psenergy, -1.0);
    assert_eq!(doc.header.wfc_cutoff, 0.0);
    assert_eq!(doc.header.rho_cutoff, 0.0);
    assert_eq!(doc.header.l_max_rho, 0);
    assert_eq!(doc.header.l_local, 0);
}
