...
use quick_xml::de::from_str as raw_from_str;
use upf::{from_file, from_str, to_file, to_writer};
use upf::{model::UpfData, to_string};
...
fn writer_preserves_real_fixture_metadata() {
    ...
    assert_eq!(reparsed.full_wfc.as_ref().unwrap().ae_wfc.len(), 6);
    assert_eq!(reparsed.full_wfc.as_ref().unwrap().ps_wfc.len(), 6);
    assert_eq!(
        reparsed.full_wfc.as_ref().unwrap().ae_wfc[0].value.index,
        Some(1)
    );
}

#[test]
fn writer_surfaces_accumulated_validation_errors() {
    let xml = r#"
    <UPF version="2.0.1">
    ...
    let doc: UpfData = raw_from_str(xml).unwrap();
