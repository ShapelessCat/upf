...
fn parses_paw_and_gipaw_subtrees_from_real_examples() {
    ...
    assert_eq!(doc.full_wfc.as_ref().unwrap().ae_wfc.len(), 8);
    assert_eq!(doc.full_wfc.as_ref().unwrap().ps_wfc.len(), 8);
    assert_eq!(doc.paw.as_ref().unwrap().data_format, "2");
    assert_eq!(doc.gipaw.as_ref().unwrap().data_format, "2");
    assert_eq!(doc.gipaw.as_ref().unwrap().core_orbitals.orbitals.len(), 13);
}
