...
pub struct NumberedTag {
    /// Stable tag family name before the numeric suffix, without the trailing dot.
...
impl NumberedTag {
    /// Parse a numbered XML tag name into family name and numeric suffix.
    pub fn parse(input: &str) -> Result<Self, String> {
        let (tag_category_name, index) = input
            .rsplit_once('.')
        ...
        Ok(Self {
            tag_category_name: tag_category_name.to_string(),
            index,
        })
    }

    /// Build a numbered tag from a family name and index.
    pub fn new(tag_category_name: impl Into<String>, index: usize) -> Self {
        Self {
            tag_category_name: tag_category_name.into(),
            index,
        }
    }

    /// Return whether the tag belongs to the requested family name.
    pub fn has_tag_category_name(&self, tag_category_name: &str) -> bool {
        self.tag_category_name == tag_category_name
    }
}

impl Ord for NumberedTag {
    fn cmp(&self, other: &Self) -> Ordering {
        match self.tag_category_name.cmp(&other.tag_category_name) {
...
impl fmt::Display for NumberedTag {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}.{}", self.tag_category_name, self.index)
    }
}
...
mod tests {
    ...
    fn parses_numbered_tags() {
        ...
        assert_eq!(tag.tag_category_name, "PP_BETA");
